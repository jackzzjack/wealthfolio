...
const ActivityManagerPage = () => {
  ...
  if (isMobileViewport) {
    return (
      <Page>
        <PageHeader
          heading={t("activity.manager.title")}
          text={
            selectedAccountName
              ? t("activity.manager.addToAccount", { account: selectedAccountName })
              : t("activity.manager.createTransaction")
          }
          onBack={handleClose}
        />
        <PageContent>
            ...
            <CardContent className="space-y-6 p-6">
              ...
              {restrictionLevel === "blocked" && (
                <Alert>
                  ...
                  <AlertTitle>{t("activity.manager.syncedAccount")}</AlertTitle>
                  <AlertDescription>
                    {t("activity.manager.syncedAccountDescription")}
                  </AlertDescription>
                </Alert>
              )}
              ...
              {restrictionLevel === "limited" && selectedAccount && (
                <Alert>
                  <Icons.Info className="h-4 w-4" />
                  <AlertTitle>{t("activity.manager.holdingsTrackingMode")}</AlertTitle>
                  <AlertDescription className="flex flex-col gap-2">
                    <span>
                      {t("activity.manager.holdingsTrackingDescription")}
                    </span>
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    >
                      ...
                      {t("activity.manager.goToAccount")}
                    </Button>
                  </AlertDescription>
                </Alert>
              )}
              ...
              {isEditing && selectedType && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground">{t("activity.manager.activityType")}</span>
