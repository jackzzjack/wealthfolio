...
export const ImportPreviewTable = ({
  ...
  const filters = [
    {
      ...
      options: [
        ...
        { label: t("activity.dataGrid.status.valid"), value: "true" },
      ],
    },
    {
      id: "activityType",
  ...
  const table = useReactTable({
    data: activities,
    columns: getColumns(accounts, baseCurrency, t),
    state: {
      sorting,
      columnFilters,
      columnVisibility,
...
function getColumns(
  ...
  return [
    ...
    },
    {
      id: "isValid",
      accessorKey: "isValid",
      header: () => <span className="sr-only">{t("activity.viewControls.status")}</span>,
      cell: ({ row }) => {
        ...
        return isValid ? (
          ...
          <TooltipProvider>
            <Tooltip delayDuration={30}>
              ...
              >
                <h4 className="mb-2 font-medium">{t("activity.importPreview.validationErrors")}</h4>
                <ul className="max-h-[300px] list-disc space-y-1 overflow-y-auto pl-5 text-sm">
                  {allErrors.length > 0 ? (
                    allErrors.map((error, index) => <li key={index}>{error}</li>)
                  ) : (
                    <li>{t("activity.importPreview.invalidActivity")}</li>
                  )}
                </ul>
              </TooltipContent>
            </Tooltip>
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          className="justify-end text-right"
          column={column}
          title={t("activity.tableMobile.price")}
        />
      ),
      cell: ({ row }) => {
        const activityType = row.getValue("activityType");
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          className="justify-end text-right"
          column={column}
          title={t("activity.detailSheet.amount")}
        />
      ),
      cell: ({ row }) => {
        const activityType = row.getValue("activityType");
