        ...
        >
          <div className="flex flex-col items-center gap-3">
            <Button size="sm" onClick={handleRemoveFile} className="flex items-center gap-1.5 px-3">
              ...
              <span>{t("activity.dropzone.removeFile")}</span>
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-0.5 text-center">
          <AnimatePresence mode="wait">
            {isLoading ? (
              ...
              >
                {t("activity.dropzone.processingFile")}
              </motion.p>
            ) : file && (!isValid || error) ? (
              <motion.div
                key="error-info"
              ...
              >
                <p className="text-xs font-medium">
                  <span className="text-primary">{t("activity.dropzone.clickToUpload")}</span>{" "}
                  {t("activity.dropzone.orDrop")}
                </p>
                <p className="text-muted-foreground text-xs">{t("activity.dropzone.csvOnly")}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
