  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>
          <p>{t("account.metrics.clickToUpdateCashBalance")}</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
...
const AccountMetrics: React.FC<AccountMetricsProps> = ({
  ...
  const rows = isHoldingsMode
    ...
    : [
        ...
        {
          label: t("account.metrics.netContributions"),
          value: (
  ...
  return (
    <Card className={cn("flex flex-col", className)}>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-lg font-bold">{t("account.metrics.cashBalance")}</CardTitle>
      ...
      <CardFooter className="mt-auto flex flex-col items-start gap-1 px-3">
        {performanceError ? (
          <p className="text-muted-foreground m-0 p-0 text-xs">
            {lastUpdated && t("account.metrics.lastUpdated", { date: lastUpdated })}
          </p>
        ) : isHoldingsMode ? (
          <>
            <p className="text-muted-foreground m-0 p-0 text-xs">
              {t("account.metrics.twrMwrUnavailable")}
            </p>
            {lastUpdated && (
              <p className="text-muted-foreground m-0 p-0 text-xs">
                {t("account.metrics.lastUpdated", { date: lastUpdated })}
              </p>
            )}
          </>
        ) : (
          <p className="text-muted-foreground m-0 p-0 text-xs">
            {t("account.metrics.fromTo", { start: formattedStartDate, end: formattedEndDate })}
          </p>
        )}
      </CardFooter>
    </Card>
