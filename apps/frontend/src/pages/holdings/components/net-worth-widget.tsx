    ...
    <Card className={cn("p-4", compact && "p-3")}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-destructive text-sm font-medium">{t("holdings.netWorth.failedToLoad")}</p>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="left" className="max-w-[250px]">
                  <p className="text-sm">
                    {t("holdings.netWorth.staleValuations")}
                    {netWorthData?.oldestValuationDate && (
                      <> {t("holdings.netWorth.lastUpdate")} {formatDate(netWorthData.oldestValuationDate)}</>
                    )}
                  </p>
                  {netWorthData && netWorthData.staleAssets.length > 0 && (
                    <p className="text-muted-foreground mt-1 text-xs">
                      {t("holdings.netWorth.needsUpdatedValuations", { count: netWorthData.staleAssets.length })}
                    </p>
                  )}
                </TooltipContent>
              </Tooltip>
          ...
          <Collapsible open={isBreakdownOpen} onOpenChange={setIsBreakdownOpen}>
            <CollapsibleTrigger asChild>
              ...
              >
                ...
                >
                  {t("holdings.netWorth.categoryBreakdown")}
                </span>
                <Icons.ChevronDown
                  className={cn(
